...
sbias_file = "superbias_20210407.fits"
...
def _with_bias():
    fr = findread()
    print("Making data")
    for uid in tqdm(lsuid):
        ls = fr.findreads(uid)
        cube = np.zeros((len(ls),4096,4096))
        for i in range(len(ls)):
            cube[i,4096,4096] = rpc(np.asarray(fits.getdata(ls[i]),dtype=float)-superbias).refpxcorr()
        ...
        #save the files
        hdu.writeto(join(path,uid+".bias.fits"))
...
def _no_bias():
    ...
    for uid in tqdm(lsuid):
        ...
        cube = np.zeros((len(ls),4096,4096))
        for i in range(len(ls)):
            cube[i,4096,4096] = rpc(np.float(fits.getdata(ls[i]))-bias).refpxcorr()
        hdu = fits.PrimaryHDU(data = cube)
        time = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
        ID = datetime.now().strftime("%y%m%d%H%M%S")
        hdu.header['DATE'] = (time,"Creation date")
